...
df <- read.csv("C:/Users/Usuario/Desktop/covid_abstracts.csv")
...
df_Sample$id <- seq.int(nrow(df_Sample))
final_df$id <- seq.int(nrow(final_df))


df_with_similarity <- merge(df_Sample, final_df, by.x = "id", by.y = "id")
df_with_similarity <- df_with_similarity %>% arrange(desc(Distancia_cos))


ggplot(df_with_similarity, aes(x=V1, y=title)) +
